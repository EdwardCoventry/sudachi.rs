...
pub fn register_functions(m: &Bound<PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(build_user_dic_bytes, m)?)?;
    Ok(())
}
...
/// Build user dictionary from lexicons based on the given system dictionary and return binary contents.
///
/// :param system: Path to the system dictionary.
/// :param lex: List of paths to lexicon files.
/// :param description: A description text to embed in the dictionary.
/// :return: Tuple of build report and compiled dictionary bytes.
///
/// :type system: pathlib.Path | str
/// :type lex: list[pathlib.Path | str | bytes]
/// :type description: str
#[pyfunction]
#[pyo3(
    signature = (system, lex, description=None),
    text_signature = "(system, lex, description=None) -> tuple[list[tuple[str, int, float]], bytes]",
)]
fn build_user_dic_bytes<'py>(
    py: Python<'py>,
    system: &Bound<'py, PyAny>,
    lex: &Bound<'py, PyList>,
    description: Option<&str>,
) -> PyResult<(Bound<'py, PyList>, Bound<'py, PyBytes>)> {
    let system_path = resolve_as_pypathstr(py, system)?;
    let system_dic = match as_data_source(system_path.as_ref(), system)? {
        DataSource::File(f) => {
            let resource_path = get_default_resource_dir(py)?;
    ...
    let mut out = Vec::new();
    errors::wrap(builder.resolve())?;
    errors::wrap(builder.compile(&mut out))?;

    let stats = to_stats(py, builder)?;
    let data = PyBytes::new(py, &out);
    Ok((stats, data))
}
