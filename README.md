...
This repository is a **fork-focused implementation** for Kiku.
...
- Preserve Kiku-compatible integer word ID semantics across multiple lexicons.
- Expose lexicon identity directly in Python `WordInfo` (including dictionary-form references).
- Preserve/extend user dictionary build behavior needed by Kiku migration.
...
- `word_id`: Kiku legacy packed integer (`lex_id * 10^8 + relative_word_id`)
...
This lets Kiku determine source lexicon and dictionary-form source lexicon without app-side inference.

## 2) Dictionary-form parsing and cross-lex behavior

User dictionary build parsing accepts dictionary-form values with parity to legacy workflows:

- `*` and `-1` are treated as `WordId::INVALID` (self-fallback behavior)
- cross-lex references are preserved
- legacy packed IDs (`lex_id * 10^8 + relative_id`) are accepted where relevant
...
This fork is intentionally opinionated for Kiku compatibility and migration needs.
